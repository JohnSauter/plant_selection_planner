// Existing Elements
const editPlantForm = document.querySelector('#edit-plant-form');
const plant_type_id = editPlantForm.dataset.id;

// NEW PLANT FORM
// Form Handler
const editPlantFormHandler = (event) => {
...
// Collect values from search form
  ...
  editPlant(plantCriteria);
};

// Functions within form handler
// Fetch function to find plants based on search criteria
const editPlant = async (criteria) => {
  if (criteria) {
    const response = await fetch(`/nursery_manager/api/plant_type/${plant_type_id}`, {
      method: 'PUT',
...
// Event listener
editPlantForm.addEventListener('submit', editPlantFormHandler);